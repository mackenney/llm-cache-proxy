# Step 08: Spec Tests — Admin Endpoints
...
### Phase Context
Wave 1: spec test files. All Wave 1 steps run in parallel after Wave 0 completes. This step creates comprehensive admin endpoint spec tests.

### This Step
Creates `tests/spec/admin.rs` with tests for all admin HTTP endpoints: `GET /` (health), `GET /stats`, `DELETE /stats`, `DELETE /cache`, and `GET /cache/<key>`. Tests verify response shapes, correct behavior after cache operations, and that delete operations affect only the intended data (stats reset doesn't delete entries; cache clear doesn't reset stats). Covers lcp-server/SPEC.md admin endpoint section.

## Prerequisites
- Wave 0 complete (all three of step-01, step-02, step-03)

## Files to Read Before Starting
- `tests/spec/cache_miss.rs` — follow this pattern for harness setup and `wait_for_writes` usage
- `tests/spec/mod.rs` — you will add `mod admin;`
- `crates/lcp-server/SPEC.md` — confirm response shapes for each admin endpoint (required fields, types)
- `tests/common/harness.rs` — confirm `harness.cache()` API for direct cache inspection

## Implementation

### Task 1: Create tests/spec/admin.rs
...
async fn test_stats_returns_required_fields() {
    ...
    assert!(body["misses"].is_number(), "stats must have 'misses' integer");
    assert!(
        body["bytes_served_from_cache"].is_number(),
        "stats must have 'bytes_served_from_cache' integer"
    );
    assert!(body["entries"].is_number(), "stats must have 'entries' integer");
    assert!(body["by_model"].is_object(), "stats must have 'by_model' object");
}
...
async fn test_stats_after_cache_operations() {
    ...
    assert!(
        stats_after_hit["bytes_served_from_cache"].as_u64().unwrap_or(0) > 0,
...
async fn test_delete_stats_resets_counters_not_entries() {
    ...
    assert_eq!(
        stats_after["bytes_served_from_cache"].as_u64().unwrap_or(999),
        0,
    ...
    assert_eq!(
        entries.len(),
        1,
        "cache entry must survive DELETE /stats"
    );
}
...
async fn test_get_cache_entry_returns_full_exchange() {
    ...
    let key = resp
        .headers()
        .get("x-lcp-key")
        .expect("x-lcp-key must be present on MISS")
        .to_str()
        .unwrap()
        .to_string();
    let _ = resp.bytes().await.unwrap();
    ...
    assert!(body["created_at"].is_string(), "entry must have 'created_at'");
...
async fn test_get_cache_entry_unknown_key_returns_404() {
    ...
    let resp = client
        ...
        .await
        .unwrap();
    assert_eq!(resp.status(), 404, "GET /cache/<nonexistent-key> must return 404");
}
```

### Task 2: Register in tests/spec/mod.rs

Add `mod admin;` to `tests/spec/mod.rs`:
...
- [ ] `cargo nextest run --test spec test_health_returns_status_ok` exits 0
- [ ] `cargo nextest run --test spec test_stats_returns_required_fields` exits 0
- [ ] `cargo nextest run --test spec test_stats_after_cache_operations` exits 0
- [ ] `cargo nextest run --test spec test_delete_stats_resets_counters_not_entries` exits 0
- [ ] `cargo nextest run --test spec test_delete_cache_clears_entries_not_stats` exits 0
- [ ] `cargo nextest run --test spec test_get_cache_entry_returns_full_exchange` exits 0
- [ ] `cargo nextest run --test spec test_get_cache_entry_unknown_key_returns_404` exits 0
- [ ] `cargo nextest run --test spec` exits 0 (no regressions)

## Reviewer Instructions

You are reviewing Step 08 implementation. Verify:

1. Run `cargo nextest run --test spec test_health test_stats test_delete test_get_cache_entry` — must exit 0, 7 tests passed
2. Run `cargo nextest run --test spec` — must exit 0 (all spec tests pass)
3. Run `grep 'mod admin' tests/spec/mod.rs` — must produce output

Report: "PASS" with each criterion confirmed, or "FAIL: <criterion> — <what's wrong> — expected: <what>"

## Rollback
If this step needs to be reverted: delete `tests/spec/admin.rs` and remove `mod admin;` from `tests/spec/mod.rs`.
